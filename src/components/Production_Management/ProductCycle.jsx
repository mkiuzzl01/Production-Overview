        ...
        {productCycle?.map((item, idx) => (
          <div className="bg-red-200 p-2 md:p-5 rounded-md" key={idx}>
            <h1 className="flex items-center gap-2 md:text-3xl font-bold">
              {item?.item === "CycleTime" || item?.item === "Throughput" ? (
                <IoIosTimer className="md:text-2xl text-green-700"></IoIosTimer>
              ) : (
                <BsFillSignYieldFill className="md:text-2xl text-purple-500"></BsFillSignYieldFill>
              )}{" "}
              {item.item}{" "}
            </h1>
